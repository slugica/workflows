...
import { Upload, Type, ImageIcon, Video, AudioLines, Bot, Zap, Pencil, Search, Wrench, MessageSquare, Crop, Download, ScanLine, Droplets } from 'lucide-react';
  ...
  return (
    ...
    >
      <span className="w-6 h-6 rounded-md bg-[#212121] flex items-center justify-center text-[11px] group-hover:bg-[#2a2a2a] transition-colors">
        {type === 'image' ? <ImageIcon size={11} /> : type === 'video' ? <Video size={11} /> : type === 'audio' ? <AudioLines size={11} /> : type === 'prompt' ? <Type size={11} /> : type === 'import' ? <Upload size={11} /> : type === 'crop' ? <Crop size={11} /> : type === 'export' ? <Download size={11} /> : type === 'preview' ? <ScanLine size={11} /> : type === 'blur' ? <Droplets size={11} /> : <Bot size={11} />}
      </span>
